...
public:
	...
	// set transformation using a model matrix directly
	void SetTransformationsMatrix(const glm::mat4& modelMatrix);

	// utility function to draw a rope between two points
        void DrawRopeBetweenPoints(const glm::vec3& pointA, const glm::vec3& pointB, float radius = 0.08f, float lengthMultiplier = 1.0f);
...
public:
	...
	// (PrepareScene and RenderScene are already declared above)
